...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
      ...
      >
        ...
        {(!collapsed || mobileOpen) && (
          <span
            className={`relative z-10 text-[13px] font-medium tracking-wide ${active ? "opacity-100 font-bold" : "opacity-90"}`}
          >
            {name}
          </span>
        )}
  ...
  const SectionLabel = ({ label }: { label: string }) =>
    !collapsed || mobileOpen ? (
      <div
        className="px-5 mt-6 mb-2 text-[10px] font-bold uppercase tracking-wider opacity-50"
    ...
    <>
      ...
      >
        ...
        <div className="h-20 flex items-center justify-center relative border-b border-white/10 shrink-0">
          <div
            className={`flex items-center gap-3 transition-all duration-300 ${collapsed && !mobileOpen ? "scale-90" : ""}`}
          >
            <div
        ...
        <div className="p-4 border-t border-white/10 shrink-0 bg-black/10">
          ...
          >
            <div
              className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center font-bold text-sm shrink-0 border-2 border-transparent transition-colors shadow-sm"
              style={{ color: currentTheme.sidebarText }}
            >
              {getInitials()}
            </div>
            ...
            {(!collapsed || mobileOpen) && (
              <div className="flex-1 min-w-0">
                <h4
                  ...
                  style={{ color: "#FFFFFF" }}
                >
                  {isMounted ? user?.first_name : ""}
                </h4>
                <p
                  className="text-[10px] font-medium truncate opacity-70 !text-white"
                  style={{ color: "#FFFFFF" }}
                >
                  {isMounted ? formatRole(user?.roles) : ""}
                </p>
              </div>
            )}
