...
export default function RentForm({
  ...
  const { register, handleSubmit: hookFormSubmit, setValue, watch, control, setError, formState: { errors: formErrors, isSubmitting } } = useForm<PropertyFormData>({
    resolver: yupResolver(propertyListingSchema) as any,
    defaultValues: initialData ? { ...initialData, listing_type: "Rent" } : { ...getInitialFormData(), listing_type: "Rent" }
  });

  const formDataImages = watch("images");
  const formDataUtilities = watch("utilities_included") || [];
  const formDataAmenities = watch("amenities") || [];

  // Update form data when initialData changes (for edit mode)
  useEffect(() => {
    if (initialData) {
      const formVals = { ...initialData, listing_type: "Rent" };
      Object.keys(formVals).forEach(k => setValue(k as any, (formVals as any)[k]));
    }
  ...
  const handleSubmit = hookFormSubmit(submitForm);

  const getFieldError = (fieldName: string): string | undefined => { return (formErrors as any)?.[fieldName]?.message; };

  return (
    <div className="max-w-[1600px] mx-auto pb-20 fade-in-up">
      <PageHeader
        backLink={backUrl}
        title={isEditMode ? "Edit Rental" : "Add Rental"}
        subtitle={
          isEditMode ? "Update details" : "Create a new rental listing"
        }
        actions={
          <>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  {[
                    ...
                    <CheckboxButton
                      key={item.name}
                      label={item.label}
                      {...register(item.name as keyof PropertyFormData)} checked={watch(item.name as keyof PropertyFormData) as boolean}
                    />
                  ))}
                </div>
              </InputField>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {utilityOptions.map((utility) => (
                  <CheckboxButton
                    ...
                    label={utility}
                    checked={formDataUtilities.includes(utility)}
                    onChange={(e) => {
                      const checked = e.target.checked;
                      const current = watch("utilities_included") || [];
                      if (checked) {
                        setValue("utilities_included", [...current, utility]);
                      } else {
                        setValue("utilities_included", current.filter((u) => u !== utility));
                      }
                    }}
                  />
                ))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {amenityOptions.map((amenity) => (
                  <CheckboxButton
                    ...
                    name="amenities"
                    label={amenity}
                    checked={formDataAmenities.includes(amenity)}
                    onChange={(e) => {
                      const checked = e.target.checked;
                      ...
                      if (checked) {
                        ...
                        setValue("amenities", current.filter((u) => u !== amenity));
                      }
                    }}
                  />
                ))}
        ...
        >
          <FileUpload
            ...
            fileCount={
              formDataImages && Array.isArray(formDataImages)
                ? formDataImages.length
                : 0
            }
          />
          {isEditMode &&
            (!formDataImages || formDataImages.length === 0) && (
